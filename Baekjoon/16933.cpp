    ...
    while(!dq.empty()) {
        ...
        for(int d = 0; d < 4; d++) {
            ...
            if(inRange(nxt_y, nxt_x)) {
                if((cur_cnt & 1) && cur_left >= 1 && board[nxt_y][nxt_x] == '1') {
        ...
        if(((cur_cnt & 1) == 0) && cur_left >= 1) {
            dists[cur_y][cur_x][cur_left] = dists[cur_y][cur_x][cur_left] + 1;
            dq.push_back({cur_cnt+1, cur_left, cur_y, cur_x});
        }
    }
    cout << -1 << "\n";
    return 0;
